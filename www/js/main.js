// 5行目にある変数の値を1から100などに変更してみましょう
// 変更したら左上の保存ボタンを押して下さい
...
var BB = {
    stage: new PIXI.Application( 
            { backgroundColor:'0x000000'}).stage,
    renderer: null,
    screenSize: null,
    paddle: null,
    balls: [],
    ...
    addBlock: function (x, y, color) {
        ...
        var texture = PIXI.Texture.from(imgPath["block_" + color],{}, false);
    ...
    addBall: function () {
        var texture = PIXI.Texture.from(imgPath["ball"],{}, false);
    ...
    addPaddle: function () {
        var texture = PIXI.Texture.from(imgPath["paddle"],{}, false);
    ...
    reset: function () {
        ...
        var resetLabel = new PIXI.Text("RESET", { fontSize: 20, fill: "red" });
        console.log("resetLabel.width:" + resetLabel.width);
        console.log("resetLabel.height:" + resetLabel.height);
        ...
        resetLabel.on('mousedown', function(){ BB.reset() } );
        // resetLabel.on('touchstart', function(){ console.log("RESET text clicked.") } );
        // resetLabel.on('click', function(){ BB.reset() } );
        // resetLabel.on('touchstart', function(){ BB.reset() } );

        setTimeout(function () {
            resetLabel.text = "RESET"; //for Android
        }, 1000, resetLabel);

        var label = new PIXI.Text("SCORE:", { fontSize: 16, fill: "red" });
        label.position.x = 20;
        label.position.y = 20;
        BB.stage.addChild(label);
        setTimeout(function () {
            label.text = "SCORE:"; //for Android
        }, 1000, label);

        BB.scoreLabel = new PIXI.Text("0", { fontSize: 18, fill: "white" });
...
function init() {
    ...
    */
    ...
    BB.renderer = new PIXI.autoDetectRenderer( 
                    { width:BB.screenSize.width, height: BB.screenSize.height} 
                );
...
function vibrate(duration) {
    ...
    if (typeof navigator !== 'undefined' && typeof navigator.vibrate !== 'undefined') navigator.vibrate(duration);
}
